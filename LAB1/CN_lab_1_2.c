...
int main() {
  ...
  for (int i = 0; i < n; i++) {
    ...
    scanf("%9s", students[i].dob);
  }

  printf("\nStudent list:\n");
  printf("SL.No | Roll number | Name | Gender | Mobile number | Phy mark | Chem mark | Math mark | DOB \n");
  for (int i = 0; i < n; i++) {
    printf("%-3d | %-8d | %-10s | %-3c | %-10d | %-8d | %-8d | %-8d | %-8s \n",
           ...
           students[i].math_mark, students[i].dob);
  }
